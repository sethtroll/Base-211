 ...
 */
public class NightmareEvent extends ItemPlugin {
        ...
        bind("Read", (player, item, slotId) -> {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    ...
                    options("This will consume the scroll.", new DialogueOption("Use " + name + ".", () -> readScroll(player, item, slotId)), new DialogueOption("Cancel."));
                }
            });
        });
    }
    ...
    private final void readScroll(final Player player, final Item item, final int slotId) {
        ...
        inventory.deleteItem(50700,1);
        if (event_started == true ) {
            player.sendMessage("The nightmare event has already started.");
            inventory.addItem(50700,1);
            return;
        }
        int NM_NPC = 378;
        Location START = new Location(3870, 9949, 3);
        event_started = true;
        World.sendMessage(MessageType.GLOBAL_BROADCAST, "<col=00FF00><shad=000000>" + player + "</col></shad> <col=FF0040>has started a Nightmare event for the server come help</col> "+ player +".");
