 ...
 */
public class WeaponBox extends ItemPlugin {
    ...
    public void handle() {
        bind("Open", (player, item, container, slotId) -> {
            ...
            final List<WeaponBox.MysteryItem> rewards = getRewards(player);
            for (final WeaponBox.MysteryItem rewardItem : rewards) {
            ...
            player.getDialogueManager().start(new ItemChat(player, item, "You open the weapon box and find " + builder + "!"));
        });
    }

    private final List<MysteryItem> getRewards(@NotNull final Player player) {
        final ArrayList<WeaponBox.MysteryItem> list = new ArrayList<>(3);
        for (int i = 0; i < 3; i++) {
    ...
    public enum MysteryItem {
        ...
        ZARYTE_CROSSBOW(ItemId.ZARYTE_CROSSBOW, 1, 1, 8, 250),
        TUMEKINSSHADOW(ItemId.TUMEKENS_SHADOW, 1, 1, 1, 250),
        VOIDWAKER(ItemId.VOIDWAKER, 1, 1, 8, 250),
        ANCIENTGS(ItemId.ANCIENT_GODSWORD, 1, 1, 8, 250),
        RAPIER(ItemId.GHRAZI_RAPIER, 1, 1, 8, 250),
        MAUL(ItemId.ELDER_MAUL, 1, 1, 8, 250),
        LONGWORD(ItemId.VESTAS_LONGSWORD, 1, 1, 8, 250),
        ARMA(ItemId.ARMADYL_CROSSBOW, 1, 1, 8, 250),
        HAMMER(ItemId.DRAGON_WARHAMMER, 1, 1, 8, 250),
        ...
        static {
            for (final WeaponBox.MysteryItem reward : values) {
                total += reward.weight;
            }
        }
        ...
        private static MysteryItem generate() {
            ...
            int current = 0;
            for (final WeaponBox.MysteryItem it : values) {
