 ...
 */
public class PkSupplys extends ItemPlugin {
    @Override
    public void handle() {
        bind("Open", (player, item, slotId) -> {
            final PkSupplys.ItemSet set = ItemSet.get(item.getId());
            if (set == null) {
                return;
            }
            ...
            for (final int id : set.getItems()) {
                player.getInventory().addItem(id, 100);
            }
        });
    }
    ...
    public enum ItemSet {
        PK_SUPPLYS(50905, 6686, 3025, 3041, 12696, 2445, 21944);
        ...
        static {
            for (final PkSupplys.ItemSet set : all) {
                SETS.put(set.id, set);
            }
        }
