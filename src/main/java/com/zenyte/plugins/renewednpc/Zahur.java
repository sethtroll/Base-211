...
public class Zahur extends NPCPlugin {
    ...
    public static void crushSecondaries(@NotNull final Player player, @NotNull final NPC npc) {
        ...
        final boolean free = player.getMemberRank().eligibleTo(MemberRank.RUNE_MEMBER);
        if (cost == -1) {
            return;
        }
        player.getDialogueManager().start(new Dialogue(player, npc) {
    ...
    private void makeUnfinishedPotions(final Player player, final NPC npc) {
        ...
        int amount = player.getMemberRank().eligibleTo(MemberRank.DRAGON_MEMBER) ? Integer.MAX_VALUE : price == 0 ? vialsOfWater : Math.min(coins / price, vialsOfWater);
    ...
    private int getPrice(final Player player) {
        if (player.getMemberRank().eligibleTo(MemberRank.RUNE_MEMBER)) {
            return 0;
        } else if (player.getMemberRank().eligibleTo(MemberRank.ADAMANT_MEMBER)) {
            return 50;
        } else if (player.getMemberRank().eligibleTo(MemberRank.MITHRIL_MEMBER)) {
            return 75;
        } else if (player.getMemberRank().eligibleTo(MemberRank.STEEL_MEMBER)) {
            return 100;
        } else if (player.getMemberRank().eligibleTo(MemberRank.IRON_MEMBER)) {
            return 125;
        } else if (player.getMemberRank().eligibleTo(MemberRank.BRONZE_MEMBER)) {
            return 150;
        }
        return 200;
    }
