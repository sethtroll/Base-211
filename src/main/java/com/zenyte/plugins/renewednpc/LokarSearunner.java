    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                options("Sail to " + (piratesCove ? "Pirate's Cove?" : "Rellekka?"), "Yes, take me there.", "No, I'd rather not.").onOptionOne(() -> sail(player, piratesCove)).onOptionTwo(this::finish);
            }
        }));
        bind("Rellekka", (player, npc) -> sail(player, false));
        bind("Pirate's Cove", (player, npc) -> sail(player, true));
    }

    @Override
    public int[] getNPCs() {
        return new int[] { 3855, NullNpcID.NULL_3854, NpcId.LOKAR_SEARUNNER_6648, NpcId.LOKAR_SEARUNNER_9306 };
    }
}
