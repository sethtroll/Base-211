...
public class ClayOven implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (option.equals("Cook")) {
            ...
            final ObjectLinkedOpenHashSet<CookingDefinitions.CookingData> cookableSet = new ObjectLinkedOpenHashSet<>();
            loop:
            for (int i = 0; i < 28; i++) {
            ...
            if (!list.isEmpty()) {
                final ArrayList<Item> itemList = new ArrayList<>(list.size());
                for (int i : list) {
