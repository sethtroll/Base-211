...
public class CatacombsRope implements ObjectAction {
    ...
    private enum Rope {
        ...
        Rope(final int varbit, final LadderObject object, final Location destination) {
            this.varbit = varbit;
            this.object = object;
            this.destination = destination;
        }

        Rope(final LadderObject object, final Location destination) {
            this.object = object;
            this.destination = destination;
        }
    }
    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        final CatacombsRope.Rope rope = Utils.findMatching(Rope.values, r -> r.object.getId() == object.getId() && r.object.getLocation().matches(object));
        if (rope == null) {
            return;
        }
        player.lock(2);
