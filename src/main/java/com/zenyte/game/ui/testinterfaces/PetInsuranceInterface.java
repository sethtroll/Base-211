        ...
        bind("Reclaim Insured Pet", (player, slotId, itemId, option) -> {
            ...
            player.getDialogueManager().start(new Dialogue(player) {
                ...
                public void buildDialogue() {
                    options("Would you like to reclaim the <col=00080>" + NPCDefinitions.getOrThrow(pet.petId()).getName() + "</col> pet for " + (StringFormatUtil.format(PetInsurance.RECLAIM_PRICE.getAmount()) + " coins?"), "Yes.", "Nevermind.").onOptionOne(() -> {
                        if (!player.getInventory().checkSpace()) {
                            setKey(5);
                        } else {
                            setKey(10);
                            if (!player.getInventory().deleteItem(PetInsurance.RECLAIM_PRICE).isFailure()) {
