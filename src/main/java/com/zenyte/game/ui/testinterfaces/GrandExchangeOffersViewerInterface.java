...
public class GrandExchangeOffersViewerInterface extends Interface {
    ...
    public void open(Player player) {
        ...
        player.getPacketDispatcher().sendClientScript(20808, getInterface().getId() << 16 | getComponent("Sort by quantity"), 0, player.getBooleanAttribute("ge_offers_viewer_quantity_sort") ? 1 : 0);
        player.getPacketDispatcher().sendClientScript(20808, getInterface().getId() << 16 | getComponent("Sort by price"), 0, player.getBooleanAttribute("ge_offers_viewer_price_sort") ? 1 : 0);
        player.getPacketDispatcher().sendClientScript(20808, getInterface().getId() << 16 | getComponent("Sort by name"), 0, player.getBooleanAttribute("ge_offers_viewer_name_sort") ? 1 : 0);
    ...
    public static final void search(final Player player, final int itemId, final ExchangeType exchangeType, SortType sortType) {
        ...
        if (size > 0) {
            player.getPacketDispatcher().sendClientScript(20803, "Viewing offers for: <col=00FFFF>" + ItemDefinitions.getOrThrow(itemId).getName());
            player.getPacketDispatcher().sendClientScript(20805, exchangeType.ordinal());
            for (int index = 0; index < size; index++) {
                ...
                player.getPacketDispatcher().sendClientScript(20801, index, itemId, result.getItem().getAmount() - result.getAmount(), StringFormatUtil.format(price), StringFormatUtil.format(totalPrice), StringFormatUtil.formatString(result.getUsername()), (int) filled, result.getAmount(), result.getItem().getAmount());
            }
            player.getPacketDispatcher().sendClientScript(20802, size);
        } else {
            player.getPacketDispatcher().sendClientScript(20804, "No results were found with your search.");
        }
