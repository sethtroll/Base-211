...
public class Decantation implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        if (from.getId() == vial || to.getId() == vial || from.getId() == gourdVial || to.getId() == gourdVial || from.getId() == ItemId.EMPTY_CUP || to.getId() == ItemId.EMPTY_CUP) {
            ...
            if (!(consumable instanceof Drinkable drink)) return;
        ...
        if (!(fromConsumable instanceof Drinkable fromDrink)) return;
        final Consumable toConsumable = Consumable.consumables.get(to.getId());
        if (!(toConsumable instanceof Drinkable toDrink)) return;
    ...
    public ItemPair[] getMatchingPairs() {
        final ArrayList<ItemOnItemAction.ItemPair> pairs = new ArrayList<>();
        final ArrayList<Drinkable> decantablePotions = new ArrayList<>(1500);
