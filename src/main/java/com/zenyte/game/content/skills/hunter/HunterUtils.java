...
public class HunterUtils {
    ...
    public static Optional<PitfallHunterNPC> getTeasedNPC(@NotNull final Player player) {
        ...
        if (!(attribute instanceof WeakReference<?> reference)) {
    ...
    @NotNull
    public static Optional<HunterTrap> findTrap(@NotNull final TrapType trapType, @NotNull final Location location, @Nullable final Predicate<HunterDummyNPC> predicate) {
        //Deadfall traps are slightly offset.
        final int radius = trapType == TrapType.DEADFALL || trapType.isNetTrap() ? 1 : 0;
        final List<NPC> matchingNPCs = CharacterLoop.find(location, radius, NPC.class, npc -> (npc.getId() == trapType.getActiveDummyNpcId() || npc.getId() == trapType.getInactiveDummyNpcId()) && (predicate == null || npc instanceof HunterDummyNPC && predicate.test((HunterDummyNPC) npc)));
    ...
    public static NetTrapPair findNetTrapPair(@NotNull final WorldObject eitherObject) throws IllegalStateException {
        ...
        final Direction direction = youngTreeDirections.get(eitherObject.getRotation()).getCounterClockwiseDirection(isNet ? 4 : 0);
        ...
        final WorldObject alternativeObject = World.getObjectWithType(alternativeObjectCoordinates, eitherObject.getType());
