...
public class AlryTheAngler extends NPCPlugin {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            final boolean goldenTench = player.getInventory().containsItem(ItemId.GOLDEN_TENCH);
            @Override
            public void buildDialogue() {
                if (goldenTench) {
                    player(" I found this big fish...");
                    item(ItemId.GOLDEN_TENCH, "You show Alry the golden tench.");
                    ...
                    npc("I don't know, I doubt that your type's up to the task...");
                    npc("But it would be quite the amusing sight. Go on!").executeAction(() -> {
                        if (player.getSkills().getLevel(Skills.HUNTER) < 35 || player.getSkills().getLevel(Skills.FISHING) < 43) {
                            setKey(160);
