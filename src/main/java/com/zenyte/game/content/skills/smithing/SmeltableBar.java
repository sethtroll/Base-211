...
public enum SmeltableBar {
    ...
    public static final Map<Integer, SmeltableBar> DATA = new HashMap<>(VALUES.length);
    ...
    public static SmeltableBar getDataByBar(final int id) {
        return BAR_MAP.get(id);
    }

    public static SmeltableBar getData(final int id) {
        return DATA.get(id);
    }

    public static SmeltableBar getData(final int id, final boolean iron) {
        if (id == 438) {
            return BRONZE_BAR;
        } else if (iron) {
