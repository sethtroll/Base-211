...
public final class VespulaRoom extends RaidArea implements PlayerCombatPlugin {
    ...
    public void load() {
        ...
        for (int i = 0; i < 2; i++) {
            final WorldObject object = new WorldObject(ObjectId.BOIL_30070, 10, 0, this.getLocation(boilLocations[index][i]));
            boils.add(object);
    ...
    public void spawnTendrils() {
        ...
        for (int i = 0; i < 2; i++) {
            final WorldObject object = new WorldObject(ObjectId.SEPTIC_TENDRILS, 10, 0, this.getLocation(tendrilLocations[index][i]));
            tendrils.add(object);
            World.spawnObject(object);
        }
    }
