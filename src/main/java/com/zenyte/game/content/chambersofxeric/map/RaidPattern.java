...
public class RaidPattern {
    ...
    static {
        final ObjectArrayList<RaidPattern> patternList = new ObjectArrayList<>();
        for (final String code : layoutCodes) {
            final String[] split = code.split(" - ");
            final String rooms = split[0];
            final String directions = split[1];
            final ObjectArrayList<LayoutTypeRoom> l = new ObjectArrayList<>(16);
     ...
     */
    @NotNull
    public static RaidPattern random(@NotNull final String partyOwner) {
        ...
        if (owner.isPresent()) {
            ...
            if (disabledSettings != 0) {
                final ObjectArrayList<RaidPattern> listOfAvailablePatterns = new ObjectArrayList<>();
