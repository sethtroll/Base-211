...
public final class RaidFloorOverviewD extends OptionsMenuD {
    ...
    public static void open(final Player player) {
        ...
        for (int i = 0; i < map.getRaidChunks().size(); i++) {
            ...
            assert data != null;
            final String string = Utils.formatString(data.toString().toLowerCase().replace("_", " "));
    ...
    private static RaidRoom getRoomData(final int ry, final int plane) {
    ...
    public void handleClick(final int slotId) {
        ...
        if (slotId < map.getRaidChunks().size()) {
            ...
            final Optional<Location> square = Utils.findEmptySquare(tile, 15, 1, Optional.empty());
