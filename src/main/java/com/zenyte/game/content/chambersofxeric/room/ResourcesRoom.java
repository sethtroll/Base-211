...
public final class ResourcesRoom extends RaidArea implements RoomController, CycleProcessPlugin {
    ...
    public void loadRoom() {
        if (getType() == RaidRoom.RESOURCES_B) {
            ...
            final ObjectArrayList<Bat.BatType> bats = new ObjectArrayList<>();
            final Bat.BatType bestBat = getBestBat(level);
            ...
            final Bat.BatType type = bats.get(Utils.random(bats.size() - 1));
     ...
     */
    private Bat.BatType getBestBat(final int level) {
        Bat.BatType bat = null;
        for (final Bat.BatType batType : Bat.BatType.values) {
