...
public final class RaidWoodcutting extends Action {
    ...
    public boolean process() {
        if (ticks++ % 4 == 0)
            player.setAnimation(hatchet.getDefinitions().getEmote());
        return tree.exists();
    }
    ...
    public int processWithDelay() {
        ...
        addLog(player);
        if (Utils.random(7) == 0) {
            final WorldObject stump = new WorldObject(ObjectId.STUMP_29764, tree.getType(), tree.getRotation(), tree.getX(), tree.getY(), tree.getPlane());
