...
public class MountQuidamortemArea extends GreatKourend {
    static void onLeave(@NotNull final Player player) {
    ...
    public static boolean appiontAnotherPartyLeader(@NotNull final Player player) {
        final ClanChannel channel = player.getSettings().getChannel();
        if (channel == null) {
            return false;
        }
        ...
        if (raid != null) {
            final ObjectArrayList<Player> eligibleLeaders = new ObjectArrayList<>();
