...
public class Vespula extends RaidNPC<VespulaRoom> implements CombatScript {
        ...
        if (id != CRAWLING) {
            ...
            if (room.getAbyssalPortal().isUnderAttack()) {
                if (id == ENRAGED) {
                    ...
                    final List<Player> targetsInMeleeDistance = CharacterLoop.find(getLocation(), getSize() + 1, Player.class, target -> CombatUtilities.isWithinMeleeDistance(this, target) && !Utils.collides(getX(), getY(), getSize(), target.getX(), target.getY(), target.getSize()));
                    stingDelay = 2;
                    Location tile;
    ...
    public int attack(final Entity target) {
        ...
        final Location destTile = new Location(target.getLocation(), 2);
