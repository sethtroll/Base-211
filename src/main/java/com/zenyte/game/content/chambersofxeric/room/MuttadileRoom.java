...
public final class MuttadileRoom extends RaidArea implements CycleProcessPlugin {
    ...
    public void spawnEmptyTree() {
        World.spawnObject(meatTree = new WorldObject(ObjectId.MEAT_TREE, 10, 0, new Location(meatTree)));
    }
    ...
    public void loadRoom() {
        ...
        World.spawnObject(meatTree = new WorldObject(ObjectId.ROCKS_30012, 10, 0, location));
    ...
    public void process() {
        ...
        for (final Player player : raid.getPlayers()) {
            if (player.getLocation().withinDistance(tendril, 6)) {
                spawnedTendril = true;
                for (final Location tendril : tendrilSpawnLocations[index]) {
                    World.spawnObject(new WorldObject(ObjectId.NOXIOUS_TENDRILS, 10, 0, this.getLocation(tendril)));
                }
