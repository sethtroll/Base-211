...
public final class Cerberus extends NPC implements CombatScript, Spawnable {
        ...
        if (location.matches(respawnTile)) {
            ...
            if (!possibleTargets.isEmpty()) {
                ...
                WorldTasksManager.schedule(() -> {
                    final ObjectArrayList<Player> possibleRemainingTargets = new ObjectArrayList<>(getLair().getPlayers());
    ...
    private void checkMarkerCollisions(@NotNull final ImmutableLocation marker) {
        for (final Player player : getLair().getPlayers()) {
    ...
    private void spawnFlames() {
        final Location westernFire = cerberusRoom.getWesternFireLocation();
        final CerberusLair lair = GlobalAreaManager.getArea(CerberusLair.class);
    ...
    private void executeSummonedSoulsAttack(@NotNull final Entity target) {
        ...
        for (int i = randomizedSoulIds.size() - 1; i >= 0; i--) {
            ...
            final SummonedSoul soul = new SummonedSoul(soulId, cerberusRoom.getWesternSoulLocation().transform(Direction.EAST, i), this, target, i);
            soul.spawn();
    ...
    private Location translateRandomReachablePosition(@NotNull final Location location) {
        ...
        final ObjectArrayList<Location> possibleLocationsList = new ObjectArrayList<>((int) Math.pow(((MARKER_RANDOMIZATION_DISTANCE << 1) + 1), 2));
