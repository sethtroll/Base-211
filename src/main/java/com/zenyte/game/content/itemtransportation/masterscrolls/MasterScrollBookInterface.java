...
public class MasterScrollBookInterface extends Interface {
    ...
    static void removeFavourite(@NotNull final Player player) {
    ...
    static Optional<ScrollBookTeleport> getFavourite(@NotNull final Player player) {
    ...
    static int getCount(@NotNull final Item book, final int scrollItemId) {
        return book.getNumericAttribute(keyPrefix + scrollItemId).intValue();
    }

    static void setCount(@NotNull final Item book, final int scrollItemId, final int count) {
        book.setAttribute(keyPrefix + scrollItemId, count);
    }

    public static List<Item> toItemList(@NotNull final Item book) {
    ...
    private static boolean isEmpty(@NotNull final Item scrollBook) {
        final Map<String, Object> attrs = scrollBook.getAttributes();
        return attrs == null || attrs.isEmpty();
    }

    static void activate(@NotNull final Player player, @NotNull final Item book, @NotNull final ScrollBookTeleport teleport) {
        final int count = getCount(book, teleport.getScrollId());
        if (count <= 0) {
    ...
    public void open(Player player) {
        ...
        if (!(scrollBook instanceof Item book)) {
            return;
        }
    ...
    private final void setFavourite(@NotNull final Player player, @NotNull final ScrollBookTeleport teleport) {
        ...
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                final String label = Utils.formatString(teleport.toString());
