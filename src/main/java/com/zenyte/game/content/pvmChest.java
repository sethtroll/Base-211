...
import com.zenyte.game.content.crystalchest.pvmChestLoot;
 ...
 */
public class pvmChest implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (!player.getInventory().containsItem(85, 1)) {
            player.sendMessage("You need a Shiny key to unlock this chest.");
            return;
        }
        ...
        player.lock(2);
        player.getInventory().deleteItem(85, 1);
        if (player.getMemberRank().eligibleTo(MemberRank.IRON_MEMBER) && Utils.random(getChance(player)) == 0) {
           player.sendMessage(Colour.RS_GREEN.wrap("You find double the loot from the pvm chest."));
            pvmChestLoot.get(player).forEach(item -> player.getInventory().addOrDrop(item));
        }
        pvmChestLoot.get(player).forEach(item -> player.getInventory().addOrDrop(item));
    }
    ...
    public Object[] getObjects() {
        return new Object[] { ObjectId.CHEST_2827 };
    }
}
