        ...
        bind("Talk-to", (player, npc) -> {
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                public void buildDialogue() {
                    ...
                    final boolean canInsureForFree = player.getMemberRank().eligibleTo(MemberRank.ADAMANT_MEMBER) && player.getPetInsurance().getInsuredPets().size() >= 2;
                    if (canInsureForFree) {
                        ...
                        npc("Once you've insured the pet, you can reclaim it here unlimited times for a reclamation fee of " + (player.getMemberRank().eligibleTo(MemberRank.MITHRIL_MEMBER) ? "nothing." : (Utils.format(PetInsurance.RECLAIM_PRICE.getAmount()) + " coins."))).executeAction(() -> {
                            if (basePetItemId == -1) {
                                setKey(70);
                                return;
                            }
