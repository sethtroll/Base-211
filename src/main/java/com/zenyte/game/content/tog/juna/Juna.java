...
public class Juna implements ObjectAction {
    ...
    public static final WorldObject JUNA_OBJECT = new WorldObject(NullObjectID.NULL_3193, 10, 1, 3252, 9516, 2);

    @Override
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        switch(option.toLowerCase()) {
            case "talk-to":
                Dialogue greetingDialogue = player.inArea(TearsOfGuthixCaveArea.class) ? new JunaInsideGreetingDialogue(player, NpcId.JUNA) : new JunaOutsideGreetingDialogue(player, NpcId.JUNA);
