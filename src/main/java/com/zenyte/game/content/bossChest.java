...
import com.zenyte.game.content.crystalchest.BossChestLoot;
 ...
 */
public class bossChest implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (!player.getInventory().containsItem(7678, 1)) {
            player.sendMessage("You need a prize key to unlock this chest.");
            return;
        }
        ...
        player.lock(2);
        player.getInventory().deleteItem(7678, 1);
        if (player.getMemberRank().eligibleTo(MemberRank.IRON_MEMBER) && Utils.random(getChance(player)) == 0) {
            player.sendMessage(Colour.RS_GREEN.wrap("You find double the loot from the boss chest."));
            BossChestLoot.get(player).forEach(item -> player.getInventory().addOrDrop(item));
        }
        BossChestLoot.get(player).forEach(item -> player.getInventory().addOrDrop(item));
    }
    ...
    public Object[] getObjects() {
        return new Object[] { ObjectId.CHEST_2826 };
    }
}
