...
public class SymbolOfLifeActivateDialogue extends Dialogue {
    ...
    private ImmutableLocation getCreateSpawnLocation() {
        final Location baseSpawnTile = player.getLocation().random(5);
        final int size = NPCDefinitions.getOrThrow(symbolOfLife.getNpcId()).getSize();
        final Location spawnTile = Utils.findEmptySquare(baseSpawnTile, 10, size, Optional.of(location -> !player.isProjectileClipped(location, true))).orElse(player.getLocation());
