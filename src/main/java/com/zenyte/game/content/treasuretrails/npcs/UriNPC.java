...
public class UriNPC extends TreasureGuardian {
    ...
    public static void spawnUri(@NotNull final Player player, @NotNull final EmoteRequest clue) {
        final Optional<Location> square = Utils.findEmptySquare(player.getLocation(), 2, 1, Optional.of(tile -> !tile.matches(player.getLocation()) && !ProjectileUtils.isProjectileClipped(null, null, player.getLocation(), tile, true) && clue.getPolygon().contains(tile)));
    ...
    public static Optional<UriNPC> findUri(@NotNull final Player player) {
        final Object attr = player.getTemporaryAttributes().get("spawned uri npc");
        if (attr instanceof UriNPC uri) {
