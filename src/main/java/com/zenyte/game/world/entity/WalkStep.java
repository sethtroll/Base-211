...
public final class WalkStep {
    public static int getHash(final int dir, final int nextX, final int nextY, final boolean check) {
        return ((dir & 7) << 28) | (nextX & 16383) | ((nextY & 16383) << 14) | (check ? 1 << 31 : 0);
    }

    public static int getDirection(final int hash) {
        return (hash >> 28) & 7;
    }

    public static int getNextX(final int hash) {
        return hash & 16383;
    }

    public static int getNextY(final int hash) {
        return (hash >> 14) & 16383;
    }

    public static boolean check(final int hash) {
        return ((hash >> 31) & 1) == 1;
    }

    public static boolean addWalkStepsInteract(final Entity entity, final int destX, final int destY, final int maxStepsCount, final int sizeX, final int sizeY, final boolean calculate) {
    ...
    public static int[] calculatedStep(final Entity entity, int myX, int myY, final int destX, final int destY, final int lastX, final int lastY, final int sizeX, final int sizeY) {
        if (myX < destX) {
            myX++;
    ...
    private static int withinRange(final int npcX, final int npcY, final int npcSize, final int targetX, final int targetY, final int targetSize) {
    ...
    public static boolean calcFollow(final Entity entity, final Position target, final int maxStepsCount, final boolean calculate, final boolean intelligent, final boolean checkEntities) {
        if (intelligent) {
    ...
    public static boolean addWalkSteps(final Entity entity, final int destX, final int destY, final int maxStepsCount, final boolean check) {
        final int[] lastTile = entity.getLastWalkTile();
        int myX = lastTile[0];
        int myY = lastTile[1];
        int stepCount = 0;
