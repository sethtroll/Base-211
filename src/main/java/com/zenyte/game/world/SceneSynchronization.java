...
public class SceneSynchronization {
     ...
     */
    public static void forEach(@NotNull final Location tile, @NotNull final Function<Player, GamePacketEncoder> synchronizablePacketFunction) {
     ...
     */
    public static void forEachFunctional(@NotNull final Location tile, @NotNull final Consumer<Player> consumer) {
        final int chunkHashCode = Chunk.getChunkHash(tile.getX() >> 3, tile.getY() >> 3, tile.getPlane());
        CharacterLoop.forEach(tile, MAXIMUM_SYNCHRONIZATION_DISTANCE, Player.class, player -> {
            if (player.getChunksInScope().contains(chunkHashCode)) {
     ...
     */
    public static void updateScopeInScene(@NotNull final Player player) {
        ...
        final IntOpenHashSet chunksInScope = player.getChunksInScope();
