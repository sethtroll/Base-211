    ...
    public void decode(final ByteBuffer buffer, final int opcode) {
        switch (opcode) {
            case 1:
                ...
                if (size > 0) {
                    ...
                    for (int index = 0; index < size; index++) {
                        models[index] = Edenify.OBJECTS.offset(id, buffer.readUnsignedShort(), Edenify.MODELS);
            ...
            case 23:
                ...
                return;
            case 24:
                animationId = Edenify.OBJECTS.offset65535(id, buffer.readUnsignedShort(), Edenify.ANIMS);
                return;
            case 27:
                clipType = 1;
                return;
            ...
            case 77:
            case 92:
                varbit = Edenify.OBJECTS.offset65535(id, buffer.readUnsignedShort(), Edenify.VARBIT);
                varp = Edenify.OBJECTS.offset65535(id, buffer.readUnsignedShort(), Edenify.VARCLIENT);
                finalTransformation = -1;
                if (opcode == 92) {
                    finalTransformation = Edenify.OBJECTS.offset65535(id, buffer.readUnsignedShort());
                }
                size = buffer.readUnsignedByte();
                transformedIds = new int[size + 2];
                for (int index = 0; index <= size; index++) {
                    transformedIds[index] = Edenify.OBJECTS.offset65535(id, buffer.readUnsignedShort());
                }
