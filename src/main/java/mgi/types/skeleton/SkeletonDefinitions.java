...
public class SkeletonDefinitions implements Definitions {
    private static final Int2ObjectOpenHashMap<SkeletonDefinitions> definitions = new Int2ObjectOpenHashMap<>(1000);

    @Override
    public void load() {
        final Cache cache = Game.getCacheMgi();
        final Archive skeletons = cache.getArchive(ArchiveType.SKELETONS);
    ...
    public static IntOpenHashSet getLinkedFrames(final int frameMapId) {
        ...
        final ObjectIterator<Int2ObjectMap.Entry<SkeletonDefinitions>> iterator = SkeletonDefinitions.getDefinitions().int2ObjectEntrySet().fastIterator();
